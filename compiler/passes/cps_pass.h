...
struct CPSPass {
    ...
    string freshTemp() {
        return "_k" + to_string(tempCounter++);
    }
    ...
    unique_ptr<CPSExpr> transformExpr(Expr* expr, const string& k) {
        ...
        if (auto e = dynamic_cast<NumberExpr*>(expr)) {
            return make_unique<CPSCall>(
                k, vector<string>{ to_string((int)e->value) }
            );
        }
        ...
        if (auto e = dynamic_cast<BinaryExpr*>(expr)) {
            ...
            // k(x op y)
            auto body = make_unique<CPSCall>(
                k, vector<string>{ t }
            );

            // let t = x op y
            auto rhs = make_unique<CPSCall>(
                e->op,
                vector<string>{
                    getName(e->left.get()),
                    getName(e->right.get())
    ...
    string getName(Expr* e) {
        ...
        throw runtime_error("CPS error: expected variable or literal (ANF violation)");
    }
